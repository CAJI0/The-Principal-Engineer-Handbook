...
Перший batch показав три behaviors. Original board revision calibrated in expected range. New board revision needed different timing window. Substitute sensor from approved alternate supplier needed different offset curve. Firmware could handle all three, if engineer knew which case was present, loaded right constants, and validated manually. Product itself did not know. It had no owned model of manufacturing-visible calibration state.

Device identity and provisioning були гірші. Identity assigned in spreadsheet після electrical test, provisioning depended on station script writing values product did not explicitly model. Service tool could read identity if already written, але не було product contract: when identity becomes valid, who owns it, what if unit leaves line without it. Spreadsheet became state owner. Station script another. Firmware had third partial opinion via cache after first boot. Warehouse labels fourth. Коли support later asked whether returned unit had substitute sensor, nobody could answer from product record alone.
...
First field trial сказав те саме різкіше. Units installed at three customer sites: stable power, noisy long cable runs, pump close to power budget. Service calls became unreadable. Support opened service tool and saw raw developer states:
...
Ці names щось означали firmware team, але не support. Technician could see `SAFE_HOLD_3`, but not whether it meant pressure sensor mismatch, missing calibration record, update recovery guard, or field wiring problem. Tool exposed internal states instead of support-safe diagnosis.

One unit reset during update. Update design був кращий за prototype path, але recovery still assumed developer laptop and private cable. Field technician had neither. Support script said return the unit. Firmware engineer said: «It is recoverable. You just have to connect with the engineering tool and run the loader in manual mode.» Це завершило argument. Recoverable by developer in lab is not recoverable in field.

Another unit lost field logs after reset. Team treated logs as convenience surface. In field logs were evidence. Reset removed pressure readings, update attempt, configuration version and voltage warning. Evidence Before Confidence (`LAW-005`) стало product obligation when engineers were absent.

Product manager asked: «Why did manufacturing and the field break the architecture?» Mara, principal engineer, answered: «They did not break it. They revealed which realities were missing from the contract.»

Вона відокремила process details від architectural promises. Manufacturing line може вибирати station layout, operator steps, label placement and fixture timing. Architecture still must say what product state exists, who owns it, which surfaces can write it, and what evidence proves it correct.
...
Every API Is a Promise (`LAW-002`) стало uncomfortable usefully: calibration script was not «just a script» if manufacturing depended on it; service tool state view was not «just debug output» if support used it; recovery command was not «just engineering access» if field plan relied on it.

Every Dependency Is a Decision (`LAW-007`) applied to pressure fixture, station script, alternate sensor, board revision encoding, label printer, spreadsheet, service laptop, field cable and update loader. Each imported behavior, failure modes, ownership boundaries and replacement cost.

First temptation: broad manufacturing mode. One flag unlocking calibration bypasses, raw state writes, fixture commands, serial number changes and extra logs. It sounded fast; it was Global Configuration (`ANTIPATTERN-003`) wearing a factory badge. One setting would affect calibration, identity, logging, update behavior, safety holds and support diagnostics, increasing Change Radius (`VOCAB-001`, `METRIC-001`).

Mara pushed for smaller surfaces. Calibration needed product-owned record with status, version, source, evidence and validation result. Station could request calibration, write measurements and receive product-level pass/fail reason, but not create hidden state model. Identity needed lifecycle: unassigned, assigned, verified, retired. Fixture access needed surface available at assembly point. Board revision needed product-level capability description. Service diagnosis needed stable vocabulary. Update recovery needed field path without developer laptop. Logs needed enough persistence to preserve last useful evidence after reset.

Highest-consequence choices went to ADR. Smaller evidence gaps to Decision Journal entries. First field escape created Mistake Ledger entry: «If recovery works on a developer laptop, it is field recovery.» False.
...
Result was not glamorous. Unit looked the same. Pump still turned on. Enclosure barely changed. But product survived places where engineers were absent. Manufacturing no longer inferred calibration validity. Fixture received product-level errors. Board revision carried capability description without HAL Everywhere (`ANTIPATTERN-002`). Service tool showed support reasons tied to configuration, calibration, hardware, firmware, environment evidence. Logs preserved last trace across reset. Component substitution became decision with evidence. Line workaround could still happen, but if it changed product behavior it needed owner, review trigger and removal condition; otherwise Temporary Solution (`ANTIPATTERN-006`) would become permanent.

Pilot still found problems. Good pilots do. One sensor lot had narrower stable range. One support message was vague. One recovery instruction was hard under pressure. But problems were visible, owned, evidenced and recorded.

Product crossed a boundary the lab could not simulate: it became architecture for people who were not in the room when architecture was designed.

## Обговорення

Manufacturing reality and field reality are not late-stage cleanup. They are design inputs.

Lab дає unusual advantages: knowledgeable engineers, direct board access, private tools, flexible timing, forgiving setup, people who remember why strange behavior is acceptable. Manufacturing and field use remove those advantages. They ask whether product can be built repeatedly, configured correctly, calibrated safely, identified reliably, recovered without developers, diagnosed by support, and explained from evidence.

Architecture need not contain every manufacturing procedure. Manufacturing process decides how line runs. Product architecture decides what state exists, who owns it, which surfaces can change it, and what evidence proves it. Field service process decides how support works with customers. Product architecture decides which diagnosis, recovery, traceability and configuration promises that process can trust.

Repeatability is first pressure. Prototype often succeeds through skilled repetition. Manufacturing needs ordinary repetition: shift changes, fixture variation, component lots, board revisions, enclosure constraints, line timing. If step requires judgment, architecture should say which judgment belongs to person and which becomes product decision with clear result.

Calibration example: dangerous question is not «Can unit be calibrated?» Better: what does product know after calibration, who owns that state, how is it validated, what does system promise? Without answer, calibration becomes Hidden State (`SMELL-004`).
...
This chapter is not a manufacturing handbook, field-service manual, fixture-design guide, observability chapter or release-process chapter. It makes manufacturing and field assumptions explicit before pilot use depends on them.

## Інженерний принцип

Проєктуйте продукт для місць, де інженерів немає. Manufacturing and field reality require architecture to make identity, calibration, configuration, diagnostics, recovery, traceability and ownership explicit before the product depends on them.
...
5. Which assumptions live in scripts, spreadsheets, private tools or team memory?
...
8. What dependency has manufacturing or field path quietly imported?
9. What is Change Radius if assumption is wrong?
10. What evidence exists outside lab?
11. Where will future engineer find owner, contract, decision and review trigger?
