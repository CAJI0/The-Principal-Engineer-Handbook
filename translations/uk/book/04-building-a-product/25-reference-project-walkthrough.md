...
Команда називала його Field Sensor Gateway. Він sample-ив один field sensor, зберігав local configuration, передавав readings через low-power radio link і відкривав simple service tool for setup and diagnosis. First customer хотів pilot. Prototype довів, що sensor reading корисний, reporting path працює в lab, а technician може змінити basic configuration без rebuilding firmware.
...
Кожний pressure виглядав local. Firmware могла замінити hard-coded interval на setting. Manufacturing могло додати station file. Support міг отримати richer log. Service-tool owner міг додати regional toggle. Release міг додати migration. Radio owner міг налаштувати retry. Жодне рішення не було хибним окремо; разом вони створили б product, який ніхто не міг пояснити.

Перший argument: reporting interval. Один engineer хотів global `reportingInterval` у non-volatile configuration. Manufacturing його записує, service tool редагує, firmware читає, regional package задає default, special timeout — інше value. Просто, доки не зʼявилися combinations: standard — every ten minutes; regional — every thirty; battery package — longer interval plus different retry; special customer timeout був не reporting interval, а gateway-absence tolerance. One broad value виглядав би flexible, але ніхто не володів promise.

Mara запитала:

> Яку difference ми обіцяємо support, а яку лише tolerating для pilot?

Regional reporting interval став supported variant promise. Battery package відклали, бо він змінював measurement cadence, retry timing і field-support expectations. Special customer timeout став pilot exception з owner і review trigger. Global setting зник. Configuration стала owned state, а не hiding place for product difference.

Другий argument: manufacturing. Station могла записати serial identity і calibration. Проблемою була authority. Manufacturing вимірювало calibration; firmware використовувала її; service tool міг request recalibration; v1.1 migration мала її preserve; older hardware revision зберігала backup інакше; recovery могла restore firmware, але не знала, чи calibration досі відповідала hardware revision без owner. Команда назвала state owners: manufacturing володіє first measurement/evidence, firmware володіє runtime calibration state and validation rules, service tool запитує workflow, але не може писати raw product truth, release володіє migration evidence, support володіє field procedure. Device reports serial identity, hardware revision, calibration version, active configuration fingerprint і firmware version in support-safe diagnostic snapshot.

Третій argument: second board revision. Sensor offset змінився. Happy path виглядав нормально. Old calibration flow, service-tool label, event meaning і v1.0 update path тепер залежали від hardware revision. Команда майже додала flag. Mara попросила простежити Change Radius. Він зачіпав firmware validation, station programming, service-tool display, field diagnostics, release notes, migration evidence, support scripts і Event Catalog. Це був не local flag, а product promise. Команда відкрила RFC для hardware-revision and configuration compatibility across firmware, manufacturing, service tooling, QA, support і release.

Четвертий pressure: field. Три pilot units перестали reporting after v1.1 update. Два завершили migration і втратили radio contact під час first report. Один rejected configuration migration, бо regional package використав old field name. Service tool показав те саме message: update failed. Правда, але марна.
...
Пʼятий pressure: release. v1.1 змінив configuration schema. Latest lab build оновився чисто, але field units існували на v1.0 і v1.0.2. Hardware revision потребувала different calibration migration. Regional variant мав different reporting interval і radio option. Special timeout був pilot exception, а не supported variant. Rollback міг повернути executable, але втратити migrated configuration meaning, якщо source snapshot і calibration state не preserved.

Release owner запитав: «Чи можемо ми ship v1.1?» Mara запитала:
...
Prototype довів useful behavior у prototype conditions. Він не довів manufacturing repeatability, field diagnosis, regional variants, release compatibility, calibration migration, support horizon або interrupted reporting recovery. Це розрізнення із Chapter 20: successful prototype — це evidence, а не baseline.
...
Observability перетворює field behavior на usable evidence. Корисна Field Sensor Gateway evidence: reset reason, active/source/target firmware, hardware revision, configuration fingerprint, variant, calibration state, migration result, radio boundary outcome, first-report result, service-tool compatibility і recovery state. Event Catalog тримає meanings stable. Hidden State, Silent Coupling, Platform Leakage і Event Explosion — risks. Відповідь — не more events, а owned events із product meaning.

Release і upgrade paths перетворюють chain на promise. Supported baseline називає direct path v1.0.2 to v1.1 із hardware revisions, regional packages, schema v2, service tool 4.3+ і preserved identity/calibration/configuration/event/recovery state. Deferred v1.0 path потребує intermediate package. Rejected paths включають old service tools, hidden variants і factory reset as default. One Lost Packet (`FAILURE-002`), The Release We Should Have Delayed (`FAILURE-005`) і The Successful Prototype (`FAILURE-003`) усі важливі, бо missing facts і release pressure expose hidden assumptions.

Records тримають chain discoverable: ADR для baseline, RFC для compatibility proposal, Decision Journal для bounded exception, Mistake Ledger для escaped assumption, Event Catalog для event meanings, Architecture Ledger для active decisions. Discoverability (`METRIC-003`) — це те, завдяки чому future engineers не сприймають ті самі assumptions як new.

Review і freeze мають scope. Architecture Review (`RITUAL-001`) потрібен, коли Change Radius перетинає owners. Architecture Freeze (`RITUAL-002`) потрібен, коли selected decisions потребують stability for validation. Freeze називає release-critical decisions, а не whole product, і keeps exception path.

Temporary Solution (`ANTIPATTERN-006`) зʼявляється всюди: prototype shortcut, pilot timeout, support script, calibration bypass, manual recovery note. Temporary work не є соромом; йому потрібен path out.

Product стає supportable, коли decisions поєднані: promises, owners, evidence, records і revisit triggers утворюють chain. Part IV завершується тут, бо product уже не лише design problem. Це shared memory, bridge into technical leadership.

## Інженерний принцип

Будуйте product як chain of explicit decisions. Кожне decision називає promise, owner, evidence і revisit condition.

Запитуйте:

1. Що prototype насправді довів?
2. Яка product reality змінила architecture?
3. Який state потребує owner?
4. Який interface став promise?
5. Яка dependency стала support obligation?
6. Яка configuration difference є supported variant?
7. Яку unsupported combination треба назвати?
8. Яка field failure має пояснювати себе?
9. Який upgrade path promised?
10. Яка evidence підтримує release?
11. Яке decision потребує review, freeze або ledger entry?
...
4. один record, який треба оновити;
...
- Baseline корисний лише тоді, коли assumptions можна знайти.
- Good walkthrough лишає decisions, які people можуть reuse.
...
Field Sensor Gateway prototype працює: він передає sensor readings через radio path, зберігає local configuration, має simple service tool і може бути оновлений у лабораторії. Manufacturing потребує serial identity і calibration flow. Field support потребує diagnostic evidence поза developer logs. Regional і hardware variants уже існують. Firmware v1.1 змінює configuration schema. Field units існують на v1.0 і v1.0.2. Support і future engineers потребують discoverable baseline: supported, deferred, evidence.

#### Рішення

Прийняти limited supported baseline для pilot release.

Pilot baseline підтримує hardware revisions A і B, standard and regional packages, configuration schema v2, service tool 4.3+ і direct upgrade з v1.0.2 до v1.1. Upgrade з v1.0 до v1.1 потребує intermediate migration package. Battery package, unsupported regional timeout combinations і old service-tool upgrade path відкладені або відхилені.
...
Pilot baseline стає supportable. Ownership стає яснішим, hidden promises зменшуються, unsupported paths названі до того, як їх знайде support. Field diagnosis поліпшується, future engineers можуть знайти product memory. Ціна: більше validation, visible deferrals, cross-team coordination, delayed customer requests і records to maintain.

#### Розглянуті альтернативи

- Відправити prototype baseline.
- Чекати на perfect product-line architecture.
- Підтримати кожну requested configuration.
- Відкласти observability і upgrade evidence до завершення pilot.
- Розділити кожного customer в окрему firmware.
- Заморозити всю architecture, доки всі unknowns не resolved.

Відхилено, бо вони ховають assumptions, додають speculative flexibility, розширюють support surface або freeze надто широко.

## Коментар редактора

Chapter 25 closes Part IV тим, що змушує product-building chapters зустрітися всередині одного reference product. Він не повторює previous chapters; він показує, як Field Sensor Gateway decision chain торкається prototype evidence, manufacturing/field reality, variants, observability і release paths.
