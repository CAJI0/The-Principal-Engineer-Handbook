...
Team зробила розумну річ: не дала кожній feature напряму викликати vendor driver. Вона створила radio manager. Спершу це відчувалося як ????.
...
Це був Silent Coupling (`SMELL-001`). Real dependency graph не був represented by diagram. Teams coordinated by memory. Tests encoded assumptions outside product ????????. Driver change reached UI, storage, command handling, diagnostics, recovery, service tooling і release testing without obvious call path.

Hidden State (`SMELL-004`) made it worse. Manager мав own view of link state. Product state machine stored copied vendor state. UI cached derived state. Service tool kept last raw driver status. Callback could mutate state during recovery before product ??????? accepted transition.

Manager став polite version of Manager Mania (`ANTIPATTERN-004`): він coordinated behavior without owning product decision. Коли запитали, чи controller may accept outbound messages during partial link recovery, answer lived in cooperation among manager, state machine, adapter і tool, not in one ???????.
...
Answer was not «radio». Radio was a device category, not yet a ????.
...
Team drew intended ???? around product-owned control behavior. Product ??????? owned authoritative radio control state, honoring Every State Has One ??????? (`LAW-001`). Adapter could observe driver state, hold mechanism-specific handles і translate completion. It could not mutate product truth through callback.

???????? became explicit because Every API Is a Promise (`LAW-002`): product-level commands, accepted inputs, observable outcomes, completion semantics, failure categories, retry ownership, recovery handoff і lifecycle constraints.

Dependency direction changed. Runtime still had callbacks, queues and async completion, but design knowledge stopped pointing from product policy into vendor mechanism. Adapter depended on product-owned ????????; product code no longer depended on vendor headers, RTOS bits, packet types, socket structures або driver retry meanings.

That made Every Dependency Is a Decision (`LAW-007`) visible. Vendor still mattered; ???? did not pretend hardware reality disappeared. It constrained where imported knowledge could spread.
...
Translation was not just renaming. One old vendor status meant both «not connected» and «driver is starting again after reset». Product had collapsed both into «link down». Second radio separated them. Team had to decide product meaning and recovery ownership. That was ???? decision, not type conversion.
...
Migration was deliberately small: outbound product message submission. Team defined product command, completion meanings, allowed states, retry ownership, adapter translation, tests і diagnostics. They measured current Change Radius (`VOCAB-001` and `METRIC-001`) and compared it with desired surface after product ????????.
...
Architecture ???? — intentional separation of responsibility, authority and knowledge. Це stricter than module, interface, layer, process, library, service or deployment ????. Those structures can implement ????; they do not prove one exists.

Start ???? work by naming the decision. «Radio» is too broad. Useful question: which product decision must remain coherent when mechanism changes?

???? separate reasons to change. Product policy, hardware integration, vendor protocol, UI, persistence, diagnostics, manufacturing, support tooling and release compatibility have different sources of change. Durable ???? groups decisions that must move together and separates decisions that should not be forced together.

Granularity matters. Too broad ???? becomes God Module (`ANTIPATTERN-001`). Too narrow ???? creates forwarding surfaces, fragmented ownership and synchronization cost. Right size is smallest truthful separation that protects the decision at stake.
...
Back channels reveal real architecture. Rare HAL calls, debug commands, test hooks, persistence fields and hidden subscriptions show what the system relies on when intended ???????? is inconvenient.

A ???? survives change when expected change can be absorbed on one side without forcing unrelated decisions to move or leak. It does not make replacement free or ????????? eternal. Durability means ???? remains truthful under expected change.

Chapter 14 gives later Part III practices a concrete object: where authority lives, what vocabulary crosses, which dependencies are contained, which back channels remain, and what change the ???? should survive.
...
1. What product decision does this ???? protect?
...
8. Which back channels bypass intended ?????????
9. Which reasons to change belong on each side?
10. What cost does ???? add?
11. What expected change should ???? absorb?
12. What evidence shows ???? is truthful?
...
### Перемалюйте одну leaky ????

Choose one integration ????: driver wrapper, service client, protocol adapter, manufacturing path, diagnostic interface, payment integration, storage connector, device bridge or subsystem API.

Write short answers:

1. What product capability does this ???? support?
...
5. What ???????? do callers believe exists?
6. Which product vocabulary appears at ?????
7. Which platform, vendor, transport or tool terms cross ?????
...
12. Which back channels bypass intended ?????????
13. Which reasons to change belong on product side?
14. Which reasons to change belong on mechanism side?
15. What realistic future change would test ?????
16. What is current affected surface if that change happens?
17. What product-owned ???????? would reduce unrelated edits?
18. What translation belongs at edge?
19. What migration path moves one product path first?
20. What tests and diagnostics would prove ???? is being used?
21. Where should ADR live?

End with one small reviewable ???? decision: protected product decision, ??????? of authoritative state, product terms allowed to cross, mechanism terms translated at edge, first back channel to remove, first path to migrate, and evidence that will show Change Radius changed.

## Нотатник Principal Engineer

- ???? separates decisions, not files.
- Translation belongs where vocabularies meet.
- Back channels reveal real architecture.

## ADR

### Chapter ADR: Place Radio Integration Behind a Product-Owned Control ????
...
Second radio family must preserve same user-visible behavior with different integration details. Current manager is wrapper, not durable ????. Product state is split across product state, manager state, UI cache, service tool status and callback mutation. Recovery ownership unclear. Direct platform bypasses exist.

#### Decision

Define product-owned radio control ????. ???? owns product-level radio behavior: link availability, command acceptance, completion meaning, failure categories, retry ownership, recovery handoff, lifecycle, diagnostics and support-visible outcomes.
...
Assign authoritative radio control state to product ???????. Driver callbacks and adapter events report observations and completions; they do not mutate product truth outside ???????.

Place vendor operations in adapters. Adapters translate identifiers, status values, packet shapes, errors, completion, timing assumptions and diagnostics at edge. Preserve async completion without exposing vendor callback signatures as product ????????.
...
- Replace subsystem in one rewrite. Raises risk before ???? is proven.
- Postpone ???? work. Lets first vendor model embed deeper.

## Коментар редактора

Chapter 14 opens Part III by moving from knowing laws to practicing architecture. It has no primary PEAK concept by design. It uses Platform Leakage (`SMELL-005`), HAL Everywhere (`ANTIPATTERN-002`), Every State Has One ??????? (`LAW-001`), Every API Is a Promise (`LAW-002`), Every Dependency Is a Decision (`LAW-007`), Silent Coupling (`SMELL-001`), Hidden State (`SMELL-004`), Manager Mania (`ANTIPATTERN-004`), God Module (`ANTIPATTERN-001`), Change Radius (`VOCAB-001` and `METRIC-001`) and ADR (`ARTIFACT-001`).
