...
Then they named дозволений рух. Firmware could fix defects inside accepted boundary, add tests, improve diagnostics that did not change semantics, roll back unaccepted parser optimization. It could not change acceptance authority, format meaning, compatibility promise or recovery behavior without exception. Gateway could fix forwarding defects preserving framing-only role. Service tooling could improve presentation but not change product-level reasons. Manufacturing could fix scripts inside accepted package format but not invent station-only payload field.

QA, support and manufacturing could continue learning. Field evidence was not violation of freeze; it was reason freeze needed шлях винятку.
...
By meeting end, release owner still had freeze, but not slogan. It had named decisions, owners, дозволений рух, evidence thresholds, exception authority, records and exit criteria.
...
Next question: what remains allowed. дозволений рух may include bug fixes inside frozen boundary, tests, diagnostics, docs, validation, rollback of unaccepted implementation detail or compatibility fixes preserving decision.
...
Use Architecture Freeze when instability itself has become system risk: protocols, persistent formats, APIs, dependencies, ownership decisions, compatibility commitments, boundary, recovery assumptions and long support costs. Freeze the decision, not curiosity.

## Інженерний принцип

Freeze named architectural decisions, not learning. Define scope, owner, дозволений рух, шлях винятку, evidence threshold and exit criteria so stability protects next phase without hiding evidence.
...
- шлях винятку is part of freeze.
...
Team gains stable validation and clearer compatibility promises. Affected teams know дозволений рух and шлях винятку. Evidence-driven correction remains visible. Cost is explicit exception handling and repeated record updates.
...
- Route every change through Architecture Review. Too heavy for дозволений рух.
