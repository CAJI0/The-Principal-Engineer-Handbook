...
- `git status --short` before edits - passed; working tree clean.
...
## Phase 2 Publishing Review

Baseline commit before gate: `2ba44b60112bb7762ceb17698f9f09f233b2b815`

Outcome: Passed.

Publishing checks performed:

- Confirmed all Phase 2 Ukrainian target files mirror `v1.0.0:book/02-the-laws/`.
- Confirmed no English canonical files under `book/` differ from `origin/main` on this branch.
- Confirmed `translations/uk/book/02-the-laws/README.md` introduces no unsafe cross-phase links.
- Confirmed no generated `site/` output and no PDF artifact are tracked.
- Confirmed reader-facing Phase 2 files contain no placeholder, draft-only, author-note, or English section-label markers.
- Reviewed remaining English-only scan results; only deliberate short canonical labels remain: `Every API Is a Promise` and `Review habit`.

Deferred publishing items:

- Full Ukrainian site/PDF publishing remains deferred to the later Ukrainian publishing phase.
- A future full Ukrainian editorial pass may further reduce mixed technical English where the glossary allows either form.

Validation:

- `git diff --check` - passed with CRLF normalization warnings only.
- Source-to-target mapping check for Phase 2 files - passed.
- `git diff --name-only origin/main...HEAD -- book/` - passed; no English canonical source files changed.
- `npm.cmd run lint:md` - passed.
- `npm.cmd run lint:spelling` - passed.
- `python -m pip check` - passed.
...
- `npm.cmd run lint:links` - passed.
- `git -c safe.directory=D:/Projects/ThePrincipaEngineerHandbook ls-files site` - passed; no tracked `site/` output.
- `vale .` - passed for Ukrainian Phase 2 after note spelling fixes; remaining non-blocking warnings are pre-existing and outside Phase 2 scope: `CONTRIBUTING.md`, `editor/ARCHITECTURE_REVIEW_0.md`, and `editor/SOURCE_OF_TRUTH.md`.
...
- Standardized Ukrainian law-title punctuation with an em dash where the title uses an equality construction.
